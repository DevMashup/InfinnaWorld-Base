Shader "InfinnaWorld/Debug/ChunkShader"
{
   Properties
    {
        ...
        SubShader
    {
        ...
        fixed4 frag(v2f i) : SV_Target
        {
            ...
            float3 baseWorldPos = (unity_ObjectToWorld._m03_m13_m23 / 7) % 1;
            baseWorldPos.y = 1;
            if (baseWorldPos.x < 0)
                baseWorldPos.x = -baseWorldPos.x;
            if (baseWorldPos.z < 0)
                baseWorldPos.z = -baseWorldPos.z;


            fixed3 lighting = i.diff * shadow + i.ambient/2 * i.light + i.ambient/2;
            col.rgb *= lighting * baseWorldPos;
            return col;
        }
    ENDCG
    }
